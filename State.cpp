...
{
    if (useDepthBufferState)    useDepthBufferState->Release();
    if (depthReadOnlyState)     depthReadOnlyState->Release();
    if (noDepthBufferState)     noDepthBufferState->Release();
    if (cullBackState)          cullBackState->Release();
    if (cullFrontState)         cullFrontState->Release();
    if (cullNoneState)          cullNoneState->Release();
    if (noBlendingState)        noBlendingState->Release();
    if (additiveBlendingState)  additiveBlendingState->Release();
    if (anisotropic4xSampler)   anisotropic4xSampler->Release();
    if (trilinearSampler)       trilinearSampler->Release();
    if (pointSampler)           pointSampler->Release();
}

bool StateManager::CreateStates()
{
    //--------------------------------------------------------------------------------------
    // Texture Samplers
    //--------------------------------------------------------------------------------------
    ...
    if (FAILED(dm->mainD3DDevice->CreateSamplerState(&samplerDesc, &pointSampler)))
    {
    ...
    // Then create a DirectX object for your description that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateSamplerState(&samplerDesc, &trilinearSampler)))
    {
        gLastError = "Error creating point sampler";
        return false;
    }
    ...
    // Then create a DirectX object for your description that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateSamplerState(&samplerDesc, &anisotropic4xSampler)))
    {
    ...
    if (FAILED(dm->mainD3DDevice->CreateRasterizerState(&rasterizerDesc, &cullBackState)))
    {
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateRasterizerState(&rasterizerDesc, &cullFrontState)))
    {
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateRasterizerState(&rasterizerDesc, &cullNoneState)))
    {
    ...
    if (FAILED(dm->mainD3DDevice->CreateBlendState(&blendDesc, &noBlendingState)))
    {
    ...
    // Then create a DirectX object for the description that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateBlendState(&blendDesc, &additiveBlendingState)))
    {
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateDepthStencilState(&depthStencilDesc, &useDepthBufferState)))
    {
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateDepthStencilState(&depthStencilDesc, &depthReadOnlyState)))
    {
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(dm->mainD3DDevice->CreateDepthStencilState(&depthStencilDesc, &noDepthBufferState)))
    {
