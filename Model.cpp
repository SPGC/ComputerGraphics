...
void Model::Render(PerModelConstants perModelConstants, ID3D11Buffer* perModelConstantBuffer)
{
    UpdateWorldMatrix();

	perModelConstants.worldMatrix = mWorldMatrix; // Update C++ side constant buffer
    UpdateConstantBuffer(perModelConstantBuffer, perModelConstants, dm); // Send to GPU

    // Indicate that the constant buffer we just updated is for use in the vertex shader (VS) and pixel shader (PS)
    dm->mainD3DContext->VSSetConstantBuffers(1, 1, &perModelConstantBuffer); // First parameter must match constant buffer number in the shader
    dm->mainD3DContext->PSSetConstantBuffers(1, 1, &perModelConstantBuffer); // First parameter must match constant buffer number in the shader
