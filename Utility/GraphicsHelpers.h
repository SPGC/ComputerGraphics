...
template <class T>
void UpdateConstantBuffer(ID3D11Buffer* buffer, const T& bufferData, DirectXManager* dm)
{
    D3D11_MAPPED_SUBRESOURCE cb;
    dm->mainD3DContext->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &cb);
    memcpy(cb.pData, &bufferData, sizeof(T));
    dm->mainD3DContext->Unmap(buffer, 0);
}
