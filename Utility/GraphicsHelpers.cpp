...
bool LoadTexture(DirectXManager* dm, std::string filename, ID3D11Resource** texture, ID3D11ShaderResourceView** textureSRV)
{
    ...
    {
        return SUCCEEDED(DirectX::CreateDDSTextureFromFile(dm->mainD3DDevice, CA2CT(filename.c_str()), texture, textureSRV));
    }
    else
    {
        return SUCCEEDED(DirectX::CreateWICTextureFromFile(dm->mainD3DDevice, dm->mainD3DContext, CA2CT(filename.c_str()), texture, textureSRV));
    }
}
