...
Mesh::Mesh(const std::string& fileName, DirectXManager* dm, bool requireTangents /*= false*/)
{
    ...
    auto shaderSignature = ShadersManager::CreateSignatureForVertexLayout(vertexElements.data(), static_cast<int>(vertexElements.size()));
    HRESULT hr = dm->mainD3DDevice->CreateInputLayout(vertexElements.data(), static_cast<UINT>(vertexElements.size()),
    ...
    hr = dm->mainD3DDevice->CreateBuffer(&bufferDesc, &initData, &mVertexBuffer);
    ...
    hr = dm->mainD3DDevice->CreateBuffer(&bufferDesc, &initData, &mIndexBuffer);
...
void Mesh::Render()
{
    ...
    UINT offset = 0;
    dm->mainD3DContext->IASetVertexBuffers(0, 1, &mVertexBuffer, &stride, &offset);

    // Indicate the layout of vertex buffer
    dm->mainD3DContext->IASetInputLayout(mVertexLayout);

    // Set index buffer as next data source for GPU, indicate it uses 32-bit integers
    dm->mainD3DContext->IASetIndexBuffer(mIndexBuffer, DXGI_FORMAT_R32_UINT, 0);

    // Using triangle lists only in this class
    dm->mainD3DContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // Render mesh
    dm->mainD3DContext->DrawIndexed(mNumIndices, 0, 0);
}
