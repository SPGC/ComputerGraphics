...
public:
    ...
    Model(Mesh* mesh, DirectXManager* dm, CVector3 position = { 0,0,0 }, CVector3 rotation = { 0,0,0 }, float scale = 1)
        : mMesh(mesh), mPosition(position), mRotation(rotation), mScale({ scale, scale, scale }), dm(dm)
    {
    }
