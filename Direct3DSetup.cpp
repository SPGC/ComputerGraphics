...
bool DirectXManager::InitDirect3D()
{
...
// the FAILED macro to test this variable, you'll see it throughout the code - it's fairly self explanatory.
    ...
    hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, flags, 0, 0, D3D11_SDK_VERSION,
        &swapDesc, &swapChain, &mainD3DDevice, nullptr, &mainD3DContext);
    if (FAILED(hr))
    {
    ...
    hr = mainD3DDevice->CreateRenderTargetView(backBuffer, NULL, &backBufferRenderTarget);
    backBuffer->Release();
    if (FAILED(hr))
    {
    ...
    dbDesc.MiscFlags = 0;
    hr = mainD3DDevice->CreateTexture2D(&dbDesc, nullptr, &depthStencilTexture);
    if (FAILED(hr))
    {
    ...
    hr = mainD3DDevice->CreateDepthStencilView(depthStencilTexture, &dsvDesc,
        &depthStencil);
    if (FAILED(hr))
    {
...
void DirectXManager::ShutdownDirect3D()
{
    ...
    if (mainD3DContext)
    {
        mainD3DContext->ClearState(); // This line is also needed to reset the GPU before shutting down DirectX
        mainD3DContext->Release();
    }
    if (depthStencil)           depthStencil->Release();
    if (depthStencilTexture)    depthStencilTexture->Release();
    if (backBufferRenderTarget) backBufferRenderTarget->Release();
    if (swapChain)              swapChain->Release();
    if (mainD3DDevice)              mainD3DDevice->Release();
}
